...
export const defaultSettings: WebsiteSettings = {
  ...
  home_subtitle:
    '从快检设备、试剂耗材、样品处理、合格证打印到检测记录管理，帮客户把快检室真正用起来。',
  ...
  company_intro:
    '山西谷芯科技有限公司围绕食品安全快检场景，提供检测仪器、试剂耗材、合格证打印、数据管理和快检室配套建议。山西省内可根据项目情况提供上门安装指导、操作培训和售后支持，省外可远程指导或协商服务方式。',
  show_materials: true,
  show_cloud_module: true,
};

export async function fetchWebsiteSettings() {
  return fetchApi<WebsiteSettings>('/public/website/settings').catch(() => defaultSettings);
}

export async function fetchWebsiteMaterials() {
  const data = await fetchApi<{ items: WebsiteMaterial[] }>('/public/website/materials?page_size=100').catch(() => ({
    items: [],
  }));
  return data.items || [];
}
